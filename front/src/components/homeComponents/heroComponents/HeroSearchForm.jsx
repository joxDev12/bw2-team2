...
function HeroSearchForm() {
  ...
  const eventiCompatibili = eventi.filter((evento) => {
    ...
    const matchLocation =
      !locationSelezionata || evento.location === ricercaLocation.trim();
    const matchData =
      !dataSelezionata || formattaDataIso(evento.date) === dataSelezionata;
  ...
  const eventiSuggeriti =
    ricerca.trim() && !ricercaSelezionata ? eventiCompatibili.slice(0, 5) : [];
  ...
  const organizzatoriSuggeriti = eventiCompatibili
    .filter((evento) => {
      ...
      return (
        testoRicerca &&
        !ricercaSelezionata &&
        (nome.includes(testoRicerca) || username.includes(testoRicerca))
      );
    })
    .reduce((lista, evento) => {
      const giaPresente = lista.some(
        (organizzatore) => organizzatore.id === evento.organizer_id,
      );
  ...
  const locationsSuggerite =
    !testoLocation || locationSelezionata
      ? []
      : eventi
          ...
          .filter(
            (evento) =>
              !dataSelezionata ||
              formattaDataIso(evento.date) === dataSelezionata,
          )
  ...
  const dateSuggerite =
    !testoData || dataInputSelezionata
      ? []
      : eventi
          .filter((evento) => eventoContieneTesto(evento, testoRicerca))
          .filter(
            (evento) =>
              !ricercaLocation.trim() ||
              evento.location === ricercaLocation.trim(),
          )
          ...
          .filter(
            (data, index, lista) =>
              lista.findIndex((item) => item.valore === data.valore) === index,
          )
  ...
  return (
    ...
    >
      <HeroSearchField
        ...
        active={
          inputAttivo === "ricerca" &&
          (eventiSuggeriti.length > 0 || organizzatoriSuggeriti.length > 0)
        }
      ...
      <button
        className="btn btn-primary px-4 custom-rounded rounded-end-3"
        type="submit"
      >
        Cerca
      </button>
